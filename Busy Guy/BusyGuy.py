...
class BusyGuy(IStrategy):
    ...
    # @tip: set to false to plot points for past candles also.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Check if the last volume is greater than 2.5 times the average volume
        dataframe['has_great_volume'] = dataframe['volume'].iloc[-1] > (dataframe['average_volume'].iloc[-1] * great_volume_threshold)
        ...
        dataframe['current_price_distance_from_sma200'] = (dataframe['sma200'].iloc[-1] - dataframe['close'].iloc[-1]) / dataframe['sma200'].iloc[-1] * 100

       # dataframe['golden_cross'] = (
       #     (dataframe['sma50_crossed_sma200_above'].all()) &
       #     (dataframe['sma50'].iloc[-1] <= dataframe['sma200'].shift(1).iloc[-1]) # Ensure the golden cross happened on the last candle
       # )
